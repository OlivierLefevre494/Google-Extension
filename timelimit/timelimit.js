...
const categorylist = {'social media' : ["https://www.instagram","https://www.twitter.com", "https://www.X.com", "https://www.facebook.com"]}
        ...
        while (x<urllist.length) {
            ...
            if (urllist[x] in categorylist) {
                ...
                while (y<categorylist[urllist[x]].length) {
                    if (categorylist[urllist[x]][y]=="https://www.instagram"){
                        console.log(Object.keys(dictionary))
                    }
                    var entry = GetDictEntry(dictionary,categorylist[urllist[x]][y])
                    if (entry!=''){
                        sum = sum + CheckTime(entry)
                    }
                    y = y+1
                }
            } else {
...
function SaveChanges() {
    ...
    while (x<groupnames.length) {
        if (groupnames[x].trim()!='' && timelimits[x].trim()!='' && timeleft[x].trim()!='') {
            blocked.push([groupnames[x], grouplist[groupnames[x]], timelimits[x], timeleft[x]])
        }
        x = x + 1
    }
...
function LoadState() {
    chrome.storage.local.get('blockedgroups').then((results) => {
        ...
        while (x<results['blockedgroups'].length) {
            ...
            AddTimeLeft(currenttimeleft,0,listofurls)
            x = x+1
        }
    })
}
  ...
  function CheckTime(websitearray) {
    ...
    while (x<websitearray.length) {
        if (websitearray[x][0]>test) {
            ...
            console.log(test<websitearray[x][0])
        }
        x = x+1
    }
    return sum/1000
...
function GetDictEntry(dict, string) {
    ...
    let x = 0
    while (x<keys.length) {
        console.log("Testing" + keys[x] + "and" + string + String(keys[x].startsWith(string)))
        if (keys[x].startsWith(string)){
